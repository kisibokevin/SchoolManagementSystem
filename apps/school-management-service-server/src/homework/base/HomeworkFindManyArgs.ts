/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { HomeworkWhereInput } from "./HomeworkWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { HomeworkOrderByInput } from "./HomeworkOrderByInput";

@ArgsType()
class HomeworkFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => HomeworkWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => HomeworkWhereInput, { nullable: true })
  @Type(() => HomeworkWhereInput)
  where?: HomeworkWhereInput;

  @ApiProperty({
    required: false,
    type: [HomeworkOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [HomeworkOrderByInput], { nullable: true })
  @Type(() => HomeworkOrderByInput)
  orderBy?: Array<HomeworkOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { HomeworkFindManyArgs as HomeworkFindManyArgs };
